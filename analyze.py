...
def collect(tdate):
    ...
    m = {k: fetch(v, tdate) for k, v in TICKERS.items()}
    s = {k: fetch(v, tdate) for k, v in SECTORS.items()}
    t = {k: fetch(v, tdate) for k, v in THEMES.items()}
    for k, v in m.items():
